...
- **File edit engine (Phase 4):** auto-generated guard tests use a thread-local override (no flaky `PI_EDIT_BLOCK_AUTO_GENERATED` env races); patch move/rename blocks auto-generated destination paths even when the destination file does not exist yet.
