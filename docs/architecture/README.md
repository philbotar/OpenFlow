...
2. [`diagrams/layers-current-vs-target.mmd`](diagrams/layers-current-vs-target.mmd) — visual current vs target seams.
3. [`threading-concurrency.md`](threading-concurrency.md) — when changing run lifecycle, I/O, or parallelism.
...
Orchestration owns runtime execution state. Domain owns model and invariants. Provider adapters own wire transport.
