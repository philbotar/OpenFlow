...
OAuth credentials are plaintext in the local OpenFlow `settings.json`, matching stored provider API keys. Normal settings IPC and logs redact them. Use Settings → OpenAI Codex → **Disconnect** to delete them.

## Provider readiness

- API-key providers resolve transient input, then stored profile key, then their environment variable.
- OpenAI Codex requires a stored refreshable ChatGPT login and ignores `OPENAI_API_KEY`.
