...
pub struct FileProjectWorkflowStore;

impl ProjectWorkflowStore for FileProjectWorkflowStore {
    ...
    fn save_all(&self, project_root: &Path, workflows: &[Workflow]) -> io::Result<()> {
        save_project_workflows(project_root, workflows)
    }

    fn delete(&self, project_root: &Path, workflow_id: &str) -> io::Result<()> {
        delete_project_workflow(project_root, workflow_id)
    }
}
