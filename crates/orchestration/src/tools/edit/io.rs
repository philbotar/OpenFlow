...
impl EditIo {
    ...
    fn finalize_after_disk_write(
        ...
        if let Some(ledger) = &self.ledger {
            let diff_summary = match (&old_normalized, &disk_normalized) {
                (Some(old), Some(new)) if old != new => Some(summarize_diff(
                    &generate_diff_string(old, new, 2).diff,
                    8,
                )),
                (None, Some(new)) if !new.is_empty() => Some(summarize_diff(
                    &generate_diff_string("", new, 2).diff,
                    8,
                )),
                _ => None,
            };
    ...
    pub fn write_text_create(&self, user_path: &str, content: &str) -> Result<WriteTextOutcome, EditIoError> {
