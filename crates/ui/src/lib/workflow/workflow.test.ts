...
describe("chatNavigationForNode", () => {
  test("routes live nodes to pick and settled nodes to filter", () => {
    const layout = {
      settled: [{ nodeId: "node-a", label: "A", messages: [], status: "completed" as const }],
      live: [{ nodeId: "node-b", label: "B", messages: [], status: "awaiting_input" as const }],
    };
    expect(chatNavigationForNode(layout, "node-b")).toEqual({ mode: "live", nodeId: "node-b" });
    expect(chatNavigationForNode(layout, "node-a")).toEqual({ mode: "settled", nodeId: "node-a" });
    expect(chatNavigationForNode(layout, "missing")).toBeNull();
  });
});
