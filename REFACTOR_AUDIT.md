...
### engine
- `crates/engine/src/ports/inbound.rs` **(whole file)** — `HumanInput`, `HumanInputPort`, `ToolApprovalInput`, `ToolApprovalPort`, plus their 2 trait impls in `execution/interactive_engine/mod.rs:681-697`. The real resume path (`orchestration/src/run/execution/drive/interaction.rs:260,292`) calls `engine.on_human_input(...)`/`on_tool_decision(...)` as **inherent methods**, bypassing this trait entirely. Highest-confidence single deletion in the audit.
- `crates/engine/src/template/mod.rs:58,69,73,77` — `Template::instantiate`, `is_field_locked`, `lock_field`, `unlock_field` — only called from their own unit tests; the field-locking feature isn't used by the real authoring flow.
- `crates/engine/src/tools/config.rs:42` — `NodeToolConfig::is_enabled` — hardcoded `true` (no `enabled` field exists), called only by its own tests. Leftover stub from a removed toggle.
- `crates/engine/src/execution/subagents.rs:50` — `subagents_for_node` — `pub` but only ever called once, internally, in the same file.
- `lib.rs` re-export audit: ~22 of 109 re-exported names have zero downstream references — `is_clarifying_question`, `build_agent_request`, `build_node_input`, `build_system_messages`, `build_upstream_map`, `collect_checkpoint_node_ids`, `merge_shared_context`, `upstream_changed_files`, `upstream_reads`, `NODE_RUNTIME_PREAMBLE`, `default_structured_output_schema`, `tool_decision_for_call`, `CallSubagentArgs`, `NodeInvocationContext`, `SubagentDeclaration`, `ToolDecision` are confidently internal-only. `CheckpointError`/`EngineInputError` cross the boundary but orchestration only ever calls `.to_string()` on them, never matches a variant.
- ⚠️ CI gates this crate's entire public API via `cargo +nightly public-api` against `crates/engine/tests/snapshots/public_api.txt` — any trim here needs that snapshot regenerated.
